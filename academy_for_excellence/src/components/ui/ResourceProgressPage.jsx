...
const ResourceProgressPage = () => {
  ...
  const fetchResourceById = async () => {
    try {
      // ✅ Build base URL then add params
      let url = `${BACKEND_URL}/api/Resources?$filter=no eq '${id}'`;
      url = appendEnvironmentParams(url);

      console.log('[DEBUG] Fetching resource by ID from:', url);
  ...
  const fetchResourceByEmail = async (email) => {
    try {
      // ✅ Build base URL then add params
      let url = `${BACKEND_URL}/api/Resources?$filter=eMail eq '${email}'`;
      url = appendEnvironmentParams(url);

      console.log('[DEBUG] Fetching resource by email from:', url);
