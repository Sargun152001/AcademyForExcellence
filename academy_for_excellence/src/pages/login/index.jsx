...
const LoginPage = () => {
  ...
  const getUserResourceByEmail = async (email) => {
    try {
      ...
      // Validate
    if (!env || !companyId) {
      console.error('[DEBUG] Missing environment or company ID in localStorage');
      return null;
    }

      // ✅ Use correct field name from OData (camelCase eMail)
      const url = `${BACKEND_URL}/api/Resources?env=${selectedEnv}&companyId=${selectedCompany}&$filter=eMail eq '${email}'`;
  ...
  useEffect(() => {
    const loadEnvironments = async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/bc/environments`);
        const data = await res.json();
  ...
  // Load companies when an environment is selected
  useEffect(() => {
    ...
    const loadCompanies = async () => {
      try {
        ...
        const res = await fetch(`${BACKEND_URL}/bc/companies?env=${selectedEnv}`);
        const data = await res.json();
              ...
              >
                ...
                {environments.map((env) => (
                  <option key={env.id} value={env.name}>
                    {env.friendlyName || env.name} ({env.type})
                  </option>
                ))}
              </select>
