...
// Add this near the top of your API file after the constants
/**
 * Get environment and company from localStorage and append to URL
 */
const appendEnvironmentParams = (url) => {
  const environment = localStorage.getItem('selectedEnvironment') || 'prod';
  const companyId = localStorage.getItem('selectedCompanyId') || '';

  const separator = url.includes('?') ? '&' : '?';
  return `${url}${separator}env=${encodeURIComponent(environment)}&companyId=${encodeURIComponent(companyId)}`;
};
...
const apiCall = async (method, endpoint, data = null, retryCount = 0) => {
  ...
  try {
    ...
    // Append environment and company params to endpoint
    const fullEndpoint = appendEnvironmentParams(endpoint);


    const config = { method, url: fullEndpoint };
...
export const submitCourseFeedback = async (bookingId, courseId, feedbackData, resourceEmail) => {
  try {
    ...
    // ✅ Apply helper function to add environment and company params
    let url = `${BACKEND_URL}/api/courseFeedbacks`;
...
export const getCourses = async (filters = {}) => {
  try {
    ...
    let url = `${BACKEND_URL}/api/Courses?$expand=Curriculams${qs}`;
...
export const getRecommendedCoursesForUsers = async (userId) => {
  try {
    ...
    let url = `${BACKEND_URL}/api/recommendedCourses?${filterQuery}`;
...
export const getSkillProgress = async (resourceId) => {
  try {
    ...
    let url = `${BACKEND_URL}/api/resSkillProgresss?${filterQuery}`;

    // Add environment and company params
    url = appendEnvironmentParams(url);

    console.log("📡 Full getSkillProgress URL being called:", url);
...
export const getCertificates = async (resourceId) => {
  try {
    const filterQuery = `$filter=resourceNo eq '${resourceId}'`;
    let url = `${BACKEND_URL}/api/resourceCertificatess?${filterQuery}`;

    // Add environment and company params
    url = appendEnvironmentParams(url);

    console.log("📡 Full getCertificates URL being called:", url);
...
export const getJobTargets = async (resourceCode) => {
  try {
    let url = `${BACKEND_URL}/api/JobTargets`;
...
export const getCourseAssessments = async (courseId) => {
  try {
    ...
    let url = `${BACKEND_URL}/api/courseAssessments?${filterQuery}`;
    // Add environment and company params
    url = appendEnvironmentParams(url);
...
export const submitAssessmentAnswers = async (submissionData) => {
  try {
    ...
    // ✅ Apply helper function to add environment and company params
    let url = `${BACKEND_URL}/api/courseAssessmentAnswers`;
    url = appendEnvironmentParams(url);
...
export const getDocumentSubCategories = async () => {
  try {
    // ✅ Apply helper function to add environment and company params
    let url = `${BACKEND_URL}/api/documentSubCategories`;
    url = appendEnvironmentParams(url);

    console.log("📡 Full getDocumentSubCategories URL with params:", url);
...
export const getSubmittalLines = async ({ category, subCategory } = {}) => {
  try {
    // ✅ Build base URL with query params first
    let url = `${BACKEND_URL}/api/submittalLines`;

    // Add category and subCategory as query params
    const params = new URLSearchParams();
    ...
    // ✅ Then append environment and company params
    url = appendEnvironmentParams(url);

    console.log("📡 Full getSubmittalLines URL with params:", url);
...
export const submitAssessmentResourceAnswers = async (data) => {

    const url =
    "/api/alletec/learning/v2.0/answers";

    const requests = data.answers.map(answer => {
